  ...
  const handleConfirmDelete = async () => {
    try {
      if (itemToDelete !== null) {
        const response = await fetch(API_URL+`/api/moteur/delete/${itemToDelete}`, {
          method: 'DELETE',
          headers: {
        ...
        if (response.ok) {
          setData(prevData => prevData.filter(element => element.idType !== itemToDelete));
      ...
      console.error('Erreur lors de la suppression depuis l\'API:', error);
    }
  };
      ...
      <table style={styles.table}>
        ...
        <tbody>
          {data.map((element, index) => (
            <tr key={element.idType} style={index % 2 === 0 ? styles.evenRow : styles.oddRow}>
              <td style={styles.td}>{element.idType}</td>
              <td style={styles.td}>{element.type}</td>
              <td style={{ ...styles.td, textAlign: 'center' }}>
                <button style={styles.deleteButton} onClick={() => handleDelete(element.idType)}>
                  Supprimer
                </button>
              </td>
            </tr>
